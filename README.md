# Projeto Integrador: Análise de Soluções Integradas para Organizações
## 📌 Descrição
Foi desenvolvida uma solução prática, acessível e eficiente para pequenos e médios vendedores que atuam em marketplaces como Shopee e Mercado Livre.
O objetivo é permitir que o vendedor evite erros de cálculo, entenda corretamente as taxas e consiga definir o preço ideal de venda, preservando sua margem de lucro.

## 🧩 Sobre o Projeto

Este projeto foi desenvolvido como parte do Projeto Integrador , unindo conceitos de análise de requisitos, lógica de negócios, automação, tecnologia aplicada e resolução de problemas reais do mercado.

Durante a pesquisa, observou-se que muitos vendedores cometem erros ao calcular:

- Tarifas por categoria
- Custos fixos adicionais
- Diferenças entre anúncios Clássico e Premium
- Impacto do parcelamento
- Redução inesperada da margem devido às taxas

Para resolver isso, foi criada uma planilha inteligente em Excel, com cálculos automatizados, validações e interface amigável.

## 📈 Impacto Esperado

A ferramenta permite que vendedores:

- Reduzam erros de cálculo
- Aumentem precisão nas margens
- Evitem prejuízos escondidos
- Tomem decisões mais profissionais
- Se tornem mais competitivos nos marketplaces

## 🛠️ Tecnologias e Ferramentas Utilizadas

- Microsoft Excel
- Fórmulas avançadas
- Python
- Macros/VBA
- Conceitos de UX aplicados à interface da planilha
- Lógica de formação de preço baseada em regras reais da Shopee e Mercado Livre

## 🎥 Demonstração do Projeto

Confira o vídeo de demonstração e veja como ele funciona na prática!

🔗 [Assista ao vídeo aqui](https://www.youtube.com/watch?v=CuZ3w0LIOfo)

## 👥 Integrantes

- Adonias Valdivino de Oliveira Neto
- Aline Santos Silva
- Francisco Ferreira do Nascimento Neto
- Karen Monique Zordan
- Letícia Simões Rodrigues
- Lucas Miranda das Chagas
- Tairan de Abreu Freitas

## Outras informações

**Faculdade:**  
Centro Universitário SENAC SANTO AMARO (modalidade EAD)

**Curso:**  
Tecnólogo em Análise e Desenvolvimento de Sistemas

**Disciplina:**  
Projeto Integrador: Análise de Soluções Integradas para Organizações

**Professor Coordenador:**  
Gustavo Calixto

**Professor Orientador:**  
Carlos Veríssimo

**Tutora:**  
Amanda Oliveira

