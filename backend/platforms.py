        ...
        with st.expander(f"**{plataforma['nome']}** {'✅' if plataforma['ativa'] else '❌'}"):
            ...
            with col2:
                ...
                if st.button("🗑️ Excluir", key=f"delete_{plataforma['id']}"):
                    st.session_state[f'confirm_delete_plat_{plataforma["id"]}'] = True
                    st.rerun()
                
                # Mostrar confirmação se necessário
                if st.session_state.get(f'confirm_delete_plat_{plataforma["id"]}', False):
                    st.warning(f"⚠️ Confirmar exclusão de **{plataforma['nome']}**?")
                    st.caption("⚠️ Isso também removerá todas as taxas desta plataforma!")
                    
                    col_yes, col_no = st.columns(2)
                    with col_yes:
                        if st.button("✅ Sim, excluir", key=f"confirm_yes_plat_{plataforma['id']}"):
                            success, msg = db.delete_plataforma(user_hash, plataforma['id'])
                            if success:
                                st.success(msg)
                                # Limpar estado
                                if f'confirm_delete_plat_{plataforma["id"]}' in st.session_state:
                                    del st.session_state[f'confirm_delete_plat_{plataforma["id"]}']
                                st.rerun()
                            else:
                                st.error(msg)
                    
                    with col_no:
                        if st.button("❌ Cancelar", key=f"confirm_no_plat_{plataforma['id']}"):
                            # Limpar estado
                            if f'confirm_delete_plat_{plataforma["id"]}' in st.session_state:
                                del st.session_state[f'confirm_delete_plat_{plataforma["id"]}']
                            st.rerun()
